...
# # 전처리 된 npy 파일 저장경로
# npy_directory = os.path.join(
#     current_dir, "sample/npy", player_code
# )  # 현재는 sample 로 시뮬레이션
...
y_train = modules.one_hot_encode_labels(y_train, num_classes)
...
if y_train.size > 0:
    ...
    model, history, test_loss, test_acc = modules.park_biGRU(
        x_train=x_train,
        y_train=y_train,
        x_test=x_test,
        y_test=y_test,
